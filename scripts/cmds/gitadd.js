...
module.exports = {
  ...
  onStart: async function ({ api, message, args, event }) {
    ...
    if (!fileName || !codeSource) {
      return message.reply(`⚠️ Missing Arguments!\nUse: gitadd <file name>.js <code or URL>`);
    }
    ...
    // ✅ Fill these with your GitHub credentials
    const githubToken = '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'; // <-- Put your actual token here
    const owner = 'arrafi06';                  // <-- Your GitHub username
    const repo = 'Rex-v1';                         // <-- Your GitHub repo name
    const branch = 'main';
    ...
    try {
      ...
      message.reply(`✅ Successfully added file!\n\n📂 File: ${fileName}\n📁 Path: scripts/cmds/${fileName}\n🟢 Committed to GitHub`);

    } catch (err) {
      const errorMsg = err.response?.data?.message || err.message;
      message.reply(`❌ Failed to add file!\nError: ${errorMsg}`);
    }
  }
};
